...
def validate_portfolio_request(auth_token: str, hash_value: str, timestamp_str: str) -> dict:
    """
    ...
    if not x_portfolio_auth or not x_portfolio_hash or not x_portfolio_timestamp:
        raise HTTPException(
            ...
            detail={
                ...
                'required': ['X-Portfolio-Auth', 'X-Portfolio-Hash', 'X-Portfolio-Timestamp']
            }
        )

    validation = validate_portfolio_request(x_portfolio_auth, x_portfolio_hash, x_portfolio_timestamp)
